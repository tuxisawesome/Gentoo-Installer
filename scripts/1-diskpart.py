...
system(f"parted -a optimal {device} --script mkpart primary 4mib 131mib")
system(f"parted -a optimal {device} --script name 2 boot")
print("* Making swap partition")
system(f"parted -a optimal {device} --script mkpart primary 132mib 643mib")
system(f"parted -a optimal {device} --script name 3 swap")
print("* Making rootfs")
system(f"parted -a optimal {device} --script mkpart primary 644mib -1mib")
